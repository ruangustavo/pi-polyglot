...
function input(text = source) { const result = prepareInput(text); assert.ok(result); return result; }
...
test("protected content never reaches the provider; provider failures don't leak its error", async () => {
  ...
  await assert.rejects(reviewText({ complete: async () => { throw new Error("TOKEN=secret"); } }, model, { targetLanguage: "en", nativeLanguage: "pt" }, input(), new AbortController().signal), (error: unknown) => error instanceof Error && !error.message.includes("secret"));
});
